<template>
  <div class="api-docs-container art-full-height overflow-y-auto p-4">
    <ElPageHeader content="角色管理模块 API 文档" />

    <ElAlert class="mt-4" type="info" :closable="false">
      <template #title>
        <div class="space-y-1">
          <div>本模块包含4个API接口，用于系统角色的CRUD管理</div>
          <div>• 权限要求：仅超级管理员（R_SUPER）可访问</div>
        </div>
      </template>
    </ElAlert>
    ...
    <div class="mt-6 space-y-6">
      <!-- API接口列表 -->
      <ElCard v-for="api in roleApiDocs" :key="api.name" shadow="hover">
        <template #header>
